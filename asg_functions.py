...
def snowflake_feedback(prompt_id,model_name,question,answer,source,feedback_choice,additional_feedback):
    ...
    try:
        ...
        session.sql(
        "insert into DATALABS.DLA_U_ZCHAN.TENSORINSIGHTS_RAG_FEEDBACK (log_time, prompt_id,model_name,question,answer,source,feedback_choice,additional_feedback ) values (?,?,?,?,?,?,?,?);"
...
def submit():
    ...
    with st.spinner('Thinking...'):
        ...
        # st.session_state.x = st.session_state.source
        # st.session_state.source = extract_file_names(st.session_state.source)
...
def feedback_func(question, answer, source):
    with st.container(border=True):
        feedback_options = [
        ...
        if button and feedback_option != 'Please select an option':
            ...
            # snowflake_feedback(prompt_id='1',model_name='1',question=question,answer=answer,
            # source=source,feedback_choice=feedback_option,additional_feedback=additional_feedback)
